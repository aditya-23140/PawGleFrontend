...
export default function PetMediaEditor() {
      ...
      .map((obj, index) => {
        ...
        return {
          ...
          thumbnail: obj.toDataURL
            ? obj.toDataURL({ format: "png", multiplier: 0.1 })
            : null,
        };
      })
  ...
  const fetchRecentEdits = async () => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_API_PORT}/api/auth/edited-pet-images/`,
        {
          headers: {
  ...
  const loadSavedEdit = async (edit) => {
    ...
    try {
      ...
      if (
        edit.edit_metadata &&
        edit.edit_metadata.objects &&
        edit.edit_metadata.objects.background
      ) {
      ...
      if (edit.edited_image_url) {
        ...
        const scale = Math.min(
          (canvas.width * 0.8) / imgElement.width,
          (canvas.height * 0.8) / imgElement.height
        );
      ...
      if (
        edit.edit_metadata &&
        edit.edit_metadata.objects &&
        edit.edit_metadata.objects.objects
      ) {
  ...
  const importImageToCanvas = () => {
    ...
    imgElement.onload = () => {
      ...
      const scale = Math.min(
        targetSize / imgElement.width,
        targetSize / imgElement.height
      );
  ...
  const addOverlay = (overlayType) => {
    ...
    setObjectPosition({
      x: Math.round(overlay.left),
      y: Math.round(overlay.top),
    });
    canvas.requestRenderAll();
    updateLayersList();
  };
  ...
  const removeImageBackground = async () => {
    ...
    try {
      ...
      imgElement.onload = () => {
        ...
        setObjectPosition({
          x: Math.round(newImage.left),
          y: Math.round(newImage.top),
        });
        canvas.requestRenderAll();
        updateLayersList();
  ...
  const saveEditedImage = async () => {
    ...
    try {
      ...
      const blob = await fetch(dataUrl).then((r) => r.blob());
      const editedFile = new File([blob], "edited-pet-image.png", {
        type: "image/png",
      });
      ...
      formData.append(
        "edit_metadata",
        JSON.stringify({ filters: selectedFilter, objects: canvas.toJSON() })
      );
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className={`py-2 px-4 rounded-lg transition duration-200 flex items-center gap-1 ${
                    isPanning
                      ? "bg-[var(--primaryColor)] text-[var(--textColor3)]"
                      : "bg-[var(--background2)] text-[var(--textColor)]"
                  } hover:bg-[var(--primaryColor)] hover:text-[var(--textColor3)]`}
                >
                  <Hand size={16} />
              ...
              <div className="mb-6">
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className={`w-full py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center gap-1 ${
                      !textInput.trim()
                        ? "bg-gray-500 text-gray-300 cursor-not-allowed"
                        : "bg-[var(--primary1)] text-[var(--textColor3)] hover:bg-[var(--primaryColor)]"
                    }`}
                  >
                    <Type size={16} />
                    Add Text
                  ...
                  <button
                    ...
                    className={`py-2 px-4 rounded-lg transition duration-200 ${
                      selectedFilter === "bright"
                        ? "bg-[var(--primaryColor)] text-[var(--textColor3)]"
                        : "bg-[var(--background2)] text-[var(--textColor)]"
                    } hover  text-[var(--textColor3)]" : "bg-[var(--background2)] text-[var(--textColor)]"} hover:bg-[var(--primaryColor)] hover:text-[var(--textColor3)]`}
                  >
                    Bright
                  </button>
                </div>
            ...
            {showLayersPanel && (
              <div className="mb-6">
                        ...
                        <li
                          ...
                          className={`p-2 rounded-md flex items-center justify-between ${
                            selectedObject === layer.object
                              ? "bg-[var(--primaryColor)] bg-opacity-30"
                              : "hover:bg-[var(--background)] hover:bg-opacity-30"
                          }`}
                        >
                          <div
                ...
                <div className="flex justify-between mt-2">
                  <button
                    onClick={() =>
                      selectedObject && bringLayerToFront(selectedObject)
                    }
                    disabled={!selectedObject}
                    className={`py-1 px-2 rounded-lg text-xs flex items-center gap-1 ${
                      !selectedObject
                        ? "bg-gray-500 text-gray-300 cursor-not-allowed"
                        : "bg-[var(--background2)] text-[var(--textColor)] hover:bg-[var(--primaryColor)] hover:text-[var(--textColor3)]"
                    }`}
                  >
                    <ArrowUp size={12} />
                    Bring to Front
                  ...
                  <button
                    onClick={() =>
                      selectedObject && sendLayerToBack(selectedObject)
                    }
                    disabled={!selectedObject}
                    className={`py-1 px-2 rounded-lg text-xs flex items-center gap-1 ${
                      !selectedObject
